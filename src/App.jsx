...
function App() {
  ...
  const brand = data.brands.find(b => b.brandKey === activeBrand)
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <header className="border-b border-gray-800 bg-gray-950/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-gray-500">更新于 {data.date}</div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        <div className="flex flex-wrap gap-2">
          {data.brands.map(b => (
            <button
              key={b.brandKey}
              onClick={() => setActiveBrand(b.brandKey)}
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                activeBrand === b.brandKey
                  ? 'text-white shadow-lg'
                  : 'bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-gray-200'
              }`}
              style={activeBrand === b.brandKey ? { backgroundColor: b.brandColor } : {}}
            >
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-6 pb-12">
        {brand && (
          <div>
            <h2 className="text-xl font-semibold mb-4" style={{ color: brand.brandColor }}>
              {brand.brandName} 资讯
            </h2>
